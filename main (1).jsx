...
function PromoSlider() {
  ...
  return (
    <section className="mx-auto max-w-7xl px-5 py-8 md:px-8">
      <div className="relative h-[150px] overflow-hidden rounded-[1.6rem] bg-white shadow-xl shadow-blue-900/10 ring-1 ring-slate-200 sm:h-[210px] md:h-[260px] lg:h-[300px]">
        ...
        <button
          ...
          onClick={prevSlide}
          aria-label="السابق"
          className="absolute right-3 top-1/2 z-20 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full bg-white/85 text-xl font-black text-[#234b87] shadow-lg backdrop-blur md:right-5 md:h-11 md:w-11"
        >
          ›
        </button>
        ...
        <button
          ...
          onClick={nextSlide}
          aria-label="التالي"
          className="absolute left-3 top-1/2 z-20 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full bg-white/85 text-xl font-black text-[#234b87] shadow-lg backdrop-blur md:left-5 md:h-11 md:w-11"
        >
          ‹
        </button>
          ...
          {sliderBanners.map((_, index) => (
            <button
              ...
              className={`h-2 rounded-full transition-all duration-300 ${
                current === index ? "w-7 bg-[#ff861c]" : "w-2 bg-white/85"
              }`}
            />
          ))}
        </div>
